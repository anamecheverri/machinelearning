...
I am including the code but not running it as these models take long to execute

```{r, eval=FALSE}
#Fitting a tree  accuracy: 53%
set.seed(125)
modtree <- train(classe~.,method="rpart",data=training)
print(modtree$finalModel)
predictions <- predict(modtree,newdata=testing)
predictions
...
# random forest pca
set.seed(125)
modrfpca <- train(classe~.,method="rf",data=training,preProcess="pca")
print(modrfpca$finalModel)
predictionsrfpca <- predict(modrfpca,newdata=testing)
confusionMatrix(predictionsrfpca, testing$classe)

# boost accuracy  .9961
set.seed(125)
modboost <- train(classe~.,method="gbm",data=training,verbose=FALSE)
print(modboost$finalModel)
predictionsboost <- predict(modboost,newdata=testing)
confusionMatrix(predictionsboost, testing$classe)

# lda accuracy 0.81
set.seed(125)
modlda <- train(classe~.,method="lda",data=training,verbose=FALSE)
print(modlda$finalModel)
predictionslda <- predict(modlda,newdata=testing)
confusionMatrix(predictionslda, testing$classe)

#stacking
set.seed(125)
preDF=data.frame(predictionsrf,predictionsboost,predictionslda,classe=testing$classe)
preDF
combrf <- train(classe~.,method="rf",data=preDF)
comPred <- predict(combrf,newdata=preDF)
confusionMatrix(comPred,testing$classe)
```

**Training the random forest model**  
This model predicts withh 99% accuracy. And I got the prediction for the 20 observations all correct

#######CODE BELOW#####
# random forest accuracy 0.9991  
```{r,cache=TRUE}
set.seed(125)  
my_saved_model <- "my_model.Rds"
if (file.exists(my_saved_model)) {
    # Read the model in and assign it to a variable.
    modrf <- readRDS(my_saved_model)
} else {
